...
| `agent`             | `AgentDefinition`                       | —                      | Main agent definition: `prompt` (system prompt), `appendPrompt`, `allowedTools` (exact names or trailing-`*` prefixes — gates **built-in tools only**; custom and MCP tools bypass the allow-list), `disallowedTools` (same syntax, applies to every tool including MCP), `availableSkills`, `maxTurns` (default `10`). Stale wildcards, zero-match allow-lists, and zero-tool agents all log warnings |
