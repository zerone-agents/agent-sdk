...
/** Runtime-real shape: revert writes _snapshot onto anchor messages. */
interface SnapshotMessage extends NormalizedMessageParam {
  _snapshot?: { beforeHash: string }
}

/** Seed messages with explicit ids + a _snapshot payload to verify stripping. */
function seedMessages(): SnapshotMessage[] {
  return [
...
/**
 * Structured provider stub — the summarizer is vi.mock'd below, so the
 * provider is never actually called (CONTRIBUTING: no unjustified `any`).
 */
const stubProvider: LLMProvider = {
  apiType: 'openai-completions',
  createMessage: () => Promise.reject(new Error('unused')),
}
...
describe('SessionManager.fork (issue #4)', () => {
  it('invariants: truncation includes anchor, _snapshot stripped, ids regenerated, summary + messageCount', async () => {
    ...
    expect((data!.messages[1] as SnapshotMessage)._snapshot).toBeUndefined() // stripped
...
describe('SessionManager revert / compact / append / rename / tag (issue #4)', () => {
  ...
  it('compactStream streams through the injected storage with source-revision CAS', async () => {
    const fake = await seededFake()
    const mgr = createSessionManager({ storage: fake })
    const result = await mgr.compact({ sessionId: 'src-1', provider: stubProvider })
  ...
  it('tag survives compact (issue #4 PR review)', async () => {
    const fake = new InMemorySessionStorage()
    await saveSessionTo(fake, 'src-1', seedMessages(),
      { cwd: '/w', model: 'm', tag: 'important', createdAt: '2020-01-01T00:00:00.000Z' },
